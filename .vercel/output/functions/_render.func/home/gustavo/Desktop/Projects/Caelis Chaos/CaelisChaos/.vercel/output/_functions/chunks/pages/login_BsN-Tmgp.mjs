...
const $$Login = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const addNewUser = async (user) => {
    try {
      const res = await fetch(`${{"PUBLIC_BACKEND_URL": "http://localhost:1234", "BASE_URL": "/", "MODE": "production", "DEV": false, "PROD": true, "SSR": true, "SITE": undefined, "ASSETS_PREFIX": undefined}.BACKEND_URL}/users`, {
        method: "POST",
        body: JSON.stringify({
          id: user.id,
          username: user.name,
