...
import { L as LoginPopup } from './CreateNewTopic_vrz2nTgb.mjs';

const getResponseContentById = async ({ quote }) => {
  try {
    const result = await fetch(
      `${"http://localhost:1234"}/forum/quote/${quote}`
    );
...
const getTopicById = async ({ topicId }) => {
  try {
    const res = await fetch(
      `${"http://localhost:1234"}/forum/${topicId}`
    );
...
const getResponsesById = async ({ id }) => {
  try {
    const result = await fetch(
      `${"http://localhost:1234"}/forum/response/${id}`
    );
    const data = await result.json();
    return data;
  } catch (err) {
  ...
  const postNewResponse = async () => {
    try {
      ...
      const response = await fetch(
        `${"http://localhost:1234"}/forum/response`,
        {
          method: "POST",
          headers: {
